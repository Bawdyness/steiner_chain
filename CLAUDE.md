...
- **Wachstum** (`lib/tools/wachstum.dart` + `lib/tools/wachstum/`) — Schwung-Treppe: aus einzelnen Rechen-Kacheln baut sich eine Funktion auf der `Größe × Zeit`-Ebene auf, Auto-Zoom auf hellgrünem Hilfsgerüst. **Nutzt den geteilten Rechner-Kern** (`lib/calc/`): Operatoren sind `BinOp` (+−×÷), Kachelwerte exakte `Rational`s, `checkpointValues` rechnet die Kette ohne f64-Drift. **Basis-Umschalter 10/12/24** (`SegmentedButton`) steuert Anzeige (Achsen/Kacheln/Live-Formel via `renderInBase`/`doubleToBaseDigits`) und den Ziffernsatz des Kachel-Editors; die gespeicherten Rationals sind basis-unabhängig. Der Kachel-/y₀-Editor (`_CalcEditSheet`) ist ein basis-bewusster **Mini-Rechner** (Zeichen-Keypad 0..base-1 + `. ± ⌫ AC`, Live-Vorschau, exakte `evaluate(…, base:)`-Auswertung) statt des früheren dezimalen TextFields.
- **Bidozenal-Rechner** (`lib/tools/bidozenal.dart` + `lib/tools/bidozenal/`) — wissenschaftlicher Basis-24-Rechner mit **Zwei-Schienen-Auswertung** (portiert aus dem Dozenal-Rechner): exakte BigInt-`Rational`-Schiene (Periodenerkennung, im Display überstrichen; `+ − × ÷`, ganzzahlige Potenz, `n!`/`|x|`/`1/x` bleiben exakt) und eine f64-Schiene für Trig/Hyperbel/`ln`/`log`/`√`/nicht-ganze Potenzen/Konstanten (π/e/φ/√2), markiert mit „≈" (State B; lange Perioden → State C). Eigener Glyphen-Satz (0-N als Vektorpfade) für Keypad + Display, Glyphen/Buchstaben-Toggle, Cursor-Navigation, dreiteiliger Error-Guard mit In-Place-Editieren, Inverse-Swap-Arming (Doppeltipp sin↔sin⁻¹), `RatLit` für Ans/Memory/„weiterrechnen" ohne Präzisionsverlust, AngleMode (DEG/RAD/GRD). Layout: Glyphen-Spalte links (3×8, bottom-up wie `store/glyph_preview.png`), rechts Display über paginiertem Funktions-Pad (Basis | f(x)). **Physische Tastatur** (`bidozenal/keyboard.dart`, portiert aus `main.dart` des Dozenal-Rechners): Ziffern `0-9` + `a-n` (= 10..23), Operatoren `+ − * / ^ %`, `!` (Fakultät), Klammern, `. ,`, `=`/Enter, Backspace, Esc (AC), ◀▶, kompletter Numpad. Eine Basis-24-Eigenheit: die Ziffernbuchstaben a-n belegen die natürlichen Tasten für sin/cos/…, deshalb bleiben die wissenschaftlichen Funktionen auf dem f(x)-Pad. Eingebunden über ein autofokussiertes `Focus`+`onKeyEvent` um den Canvas; Tasten-Buttons sind `canRequestFocus: false`, damit ein Tap die Tastatur-Fokus nicht klaut. Der Rechner-Kern liegt in **`lib/calc/`** (geteilte Infrastruktur, siehe unten), tool-lokal bleiben nur `bidozenal/{display,keypad,result}.dart`. Design-Fundament in `docs/bidozenal.md`; Theorie-Quelle `assets/theory/bidozenal.md` ist **hand-geschriebenes Markdown** (nicht die LyX→md-Pipeline). Glyphen-Referenz: `store/glyph_preview.png` + Wegwerf-Skript `tool/preview_glyphs.dart`.
- **Kurve** (`lib/tools/kurve.dart` + `lib/tools/kurve/`) — Funktionsplotter mit Wachstum-DNA: `f(x)` über ein Dozenal-Keypad (Ziffern 0–B + Variable `x`) eingeben, Kurve wird über die geteilte f64-Schiene (`compileF64`) pro Pixelspalte gesampelt und auf einem pan/zoombaren x-y-Plan gezeichnet (`kurve/plot_painter.dart`, eigener Painter — `WachstumLayout` passt nicht zum freien Viewport). Statt statisch *wächst* die Kurve + Fläche per Hz-Sweep links→rechts (Ticker wie Wachstum); durch steile Stellen schnell, flache langsam. Achsen-Ticks in Basis 12 (`renderInBase`, exakte `Rational`-Ticks aus `mant×12^exp`). Winkel in Radiant. Pan/Zoom via `GestureDetector.onScaleUpdate`; physische Tastatur wie beim Rechner (`x`→`VarTok` zuerst, sonst `eventForKey`).

### Geteilter Rechner-Kern (`lib/calc/`)

Da die Disziplin **keine Cross-Tool-Imports** verbietet, liegt der Rechner-Kern in `lib/calc/` (geteilte Infrastruktur wie `lib/widgets/` / `lib/scaffold/`), genutzt von **Bidozenal-Rechner** und **Kurve**: `digits` · `rational` (basis-parametrierbar: `expand(base:)`, `renderInBase`, `doubleToBaseDigits`) · `evaluator` (Token-Modell, AST, exakte + f64-Schiene; `evaluate(…, {x, base})` + `compileF64` für den Plot-Hot-Loop; Variable `VarTok`; Zahl-Parsing nimmt `base` → Bidozenal parst Basis 24, Kurve Basis 12) · `input` (`KeypadEvent`-Sealed) · `glyphs` (24-Glyphen-Painter, Subset für Basis 12) · `keyboard` (Tasten-Map; mappt bewusst **kein** `x`, das macht jedes Tool selbst). `VarTok` kollabiert die exakte Schiene (eine Funktion von x hat keinen exakten Einzelwert).
