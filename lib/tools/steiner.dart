...
class _SteinerPageState extends State<SteinerPage>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Steiner-Kette')),
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          if (isWide) {
            final maxControls = constraints.maxWidth - 240;
            final w = _controlsWidth.clamp(260.0, maxControls);
            return Row(
              children: [
                SizedBox(width: w, child: controls),
                _DragHandle(
                  onDrag: (dx) => setState(() {
                    _controlsWidth =
                        (_controlsWidth + dx).clamp(260.0, maxControls);
                  }),
                ),
